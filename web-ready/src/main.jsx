...
function AdminOpsRetention() {
  ...
  async function load() {
    try {
      ...
      if (!(me?.is_super || me?.role === 'owner')) {
        setState({ loading: false, error: null, buckets: null, keep: null, me });
        return; // will render access denied below
      }
      const r = await fetch(`${API_BASE}/admin/ops/usage/buckets`, {
        headers: { Authorization: 'Bearer ' + localStorage.getItem('token') }
      });
      const j = await r.json();
      if (!r.ok || j.ok === false) throw new Error(j.error || 'request failed');
      setState({ loading: false, error: null, buckets: j.buckets || {}, keep: j.keep || {}, me });
    } catch (e) {
  ...
  if (state.loading) {
    return <div className="card"><div className="card-body">Loading retention buckets…</div></div>;
  }

  if (!(state.me?.is_super || state.me?.role === 'owner')) {
    ...
    return (
      <div className="card">
        <div className="card-body">
          <div style={{ color: 'crimson', marginBottom: 8 }}>Failed to load: {String(state.error)}</div>
  ...
  const values = [b['<90d'] || 0, b['90-180d'] || 0, b['>180d'] || 0];
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="card-body">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 12, marginBottom: 12 }}>
            <div className="stat">
              <div className="stat-label">Keep Alerts (days)</div>
              <div className="stat-value">{state.keep?.alerts_days ?? '—'}</div>
            </div>
            <div className="stat">
              <div className="stat-label">Keep Usage (days)</div>
              <div className="stat-value">{state.keep?.usage_days ?? '—'}</div>
            </div>
            <div className="stat">
              <div className="stat-label">Total Usage Events</div>
              <div className="stat-value">{values.reduce((a, b) => a + b, 0)}</div>
            </div>
          </div>
          <Bar label={'< 90 days'} value={values[0]} max={max} />
          <Bar label={'90–180 days'} value={values[1]} max={max} />
          <Bar label={'> 180 days'} value={values[2]} max={max} />
          <div style={{ marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button onClick={load}>Refresh</button>
            <a href="/admin/ops/retention/preview" target="_blank" rel="noreferrer">API: Preview</a>
            <a href="/admin/ops/retention/run" target="_blank" rel="noreferrer">API: Run Now</a>
            <a href="/admin/ops/seed/usage?days_ago=200&count=2000" target="_blank" rel="noreferrer">API: Seed 2k @200d</a>
          </div>
        </div>
      </div>
    </div>
