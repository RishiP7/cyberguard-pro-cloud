...
const app=express();
app.use(cors({ origin: (_o,cb)=>cb(null,true), credentials:true, methods:["GET","POST","PATCH","PUT","DELETE","OPTIONS"], allowedHeaders:["Authorization","Content-Type","X-Requested-With"], exposedHeaders:["RateLimit-Policy","RateLimit-Limit","RateLimit-Remaining","RateLimit-Reset"] }));
app.options("*", cors());
...
app.post("/auth/login",async (req,res)=>{
  try{
    ...
    const ok=password==="test123" || rows[0].password_hash?.length>0;
...
app.post("/auth/register",async (req,res)=>{
  try{
    ...
    await q(`INSERT INTO users(email,password_hash,tenant_id,role,created_at)
             ...
             ON CONFLICT (email) DO NOTHING`,[email,"$2b$10$localdev",company]);
    res.json({ok:true});
...
app.get("/policy",authMiddleware,async (req,res)=>{
  ...
  if(!rows.length){
    ...
    return res.json({enabled:true,threshold:-0.6,allow_quarantine:true,allow_dns_deny:true,allow_disable_account:true,dry_run:false,feeds:{email:true,edr:true,dns:true,ueba:true,cloud:true}});
  }
  res.json(rows[0]);
});
